...
<html lang="en" dir="ltr">
   ...
   <head>
      ...
      <script  type="text/javascript" >
         ...
         function saveEdits() {
            ...
            var editElem = document.getElementById("club_history");
            var editElem1 = document.getElementById("importent_player");
        ...
        function checkEdits() {
            ...
            if(localStorage.userEdits!=null){
               document.getElementById("club_history").innerHTML = localStorage.userEdits;
               }
  
            if(localStorage.userEdits1 != null){
               document.getElementById("importent_player").innerHTML = localStorage.userEdits1; 
            }
         }
   ...
   <body onload="checkEdits()" style="background-image: url('images/background.png') ;">
     ...
     <?php if (!empty($_SESSION['username'])){?>
      ...
      <h4 class="display-4 text-center fw-bolder" style="color:rgb(25, 33, 133);">:היסטוריית המועדון</h4>
      <p contentEditable=<?php echo $_POST['edit']?> class="text-end fw-bolder text-light" name="club_history" id="club_history" dir="rtl">      
         ...
         .במהלך שנות קיומה, זכתה הקבוצה ב - 10 אליפויות ו - 16 גביעי מדינה
         .המועדון זכה חמש פעמים בדאבל (1998, 1999, 2000, 2001, 2011)
      

      

      </p>
      <h6 class="display-6 text-center fw-bolder" style="color:rgb(25, 33, 133);">:שחקנים בולטים במועדון במשך השנים</h6>
      ...
      <?php if (!empty($_SESSION['username'])){?>
       <!-- <input type="button" class="btn btn-success" value="save my edits" onclick="saveEdits();"/> -->
         ...
         $date = date('d-m-y');?>
       <form name="form" method="post" action="legacy.php">&nbsp;
            <input type="hidden" name="Hdate" value="<?php echo $date?>">
            <input type="hidden" name="userName" value="<?php echo $_SESSION['username']?>">
            <button type="submit" name="History" value ="Legacy" class="btn btn-success mt-3" onclick="saveEdits();">save my edits</button>
            </form>
      <?php } ?>
